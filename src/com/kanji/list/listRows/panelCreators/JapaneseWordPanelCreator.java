...
public class JapaneseWordPanelCreator {
	...
	public MainPanel createPanelInGivenMode(JapaneseWordInformation japaneseWord,
		...
		listElementData
				.add(new ListElementData<>(Labels.WORD_MEANING, new JapaneseWordMeaningChecker(),
						ListElementPropertyType.STRING_SHORT_WORD,
						Labels.COMBOBOX_OPTION_SEARCH_BY_WORD_MEANING)); listElementData
				.add(new ListElementData<>(Labels.WORD_IN_KANA,
						new JapaneseWordWritingsChecker(null),
						ListElementPropertyType.STRING_SHORT_WORD,
						Labels.COMBOBOX_OPTION_SEARCH_BY_KANA));

		wordMeaningText = CommonGuiElementsMaker.createShortInput(japaneseWord.getWordMeaning());
		wordMeaningText.addFocusListener(new ListPropertyChangeHandler<>(japaneseWord,
				applicationWindow.getApplicationController().getJapaneseWords(), applicationWindow,
		...
		wordMeaningText.addFocusListener(new FocusAdapter() {
			@Override public void focusGained(FocusEvent e) {
				System.out.println(wordMeaningText.getInputContext().getLocale());
				super.focusGained(e);
			}
		});
		...
		JLabel partOfSpeechLabel = GuiMaker
		...
		partOfSpeechCombobox.setSelectedItem(japaneseWord.getPartOfSpeech().getPolishMeaning());
		...
		if (listPanelViewMode.equals(ListPanelViewMode.VIEW_AND_EDIT)) {
			partOfSpeechCombobox.addItemListener(new ItemListener() {
				@Override public void itemStateChanged(ItemEvent e) {
					String newValue = (String) e.getItem(); japaneseWord
							.setPartOfSpeech(PartOfSpeech.getPartOfSpeachByPolishMeaning(newValue));
				}
			});
		}
		...
		firstRow.add(wordMeaningLabel); firstRow.add(wordMeaningText);

		addWordPanel.addElementsInColumnStartingFromColumn(wordMeaningText, 0,
				firstRow.toArray(new JComponent[] {})); addWordPanel
				.addElementsInColumnStartingFromColumn(partOfSpeechCombobox, 1, partOfSpeechLabel,
		...
		addWordPanel.addElementsInColumnStartingFromColumn(1, addKanaAndKanjiWritingsButton);

		if (listPanelViewMode.equals(ListPanelViewMode.ADD)) {
			addKanaAndKanjiWritingRow(japaneseWord, addWordPanel, null, listPanelViewMode);
		}
		else {
			addWordPanel.addElementsInColumnStartingFromColumn(1, listElements.getButtonDelete());
			if (listPanelViewMode.equals(ListPanelViewMode.VIEW_AND_EDIT)) {
				for (Map.Entry<String, List<String>> kanaToKanjiWritings : japaneseWord
						.getKanaToKanjiWritingsMap().entrySet()) {
					addKanaAndKanjiWritingRow(japaneseWord, addWordPanel, kanaToKanjiWritings,
							listPanelViewMode);
				}
	...
	public MainPanel createPanelInViewMode(JapaneseWordInformation japaneseWordInformation,
			CommonListElements commonListElements,
			RowInJapaneseWordInformations rowInJapaneseWordInformations) {

		return createPanelInGivenMode(japaneseWordInformation, ListPanelViewMode.VIEW_AND_EDIT,
				commonListElements);

	}

	private void addKanaAndKanjiWritingRow(JapaneseWordInformation japaneseWordInformation,
			MainPanel rootPanel, Map.Entry<String, List<String>> kanaAndKanjiWritingsValues,
		...
		if (kanaAndKanjiWritingsValues == null) {
			...
			kanjiTextComponents
					.add(CommonGuiElementsMaker.createShortInputWithPrompt(Prompts.KANJI_TEXT));
			...
			for (String kanji : kanaAndKanjiWritingsValues.getValue()) {
				kanjiTextComponents.add(CommonGuiElementsMaker.createShortInput(kanji));
			}
		}
		...
		kanaWritingText.addFocusListener(new FocusAdapter() {
			...
			@Override public void focusGained(FocusEvent e) {
				...
				kanaWritingText.getInputContext().selectInputMethod(
						Locale.JAPAN);
				kanaWritingText.getInputContext().setCharacterSubsets(
						new Character.Subset[] { Character.UnicodeBlock.HIRAGANA});
				super.focusGained(e);
			}
			...
			@Override public void focusLost(FocusEvent e) {
				super.focusLost(e);
				if (!already){
					kanaWritingText.getInputContext().selectInputMethod(Locale.getDefault());
				}
				System.out.println(kanaWritingText.getInputContext().getLocale());

				already = true;
			}
		});

		propertyManagersOfTextFields.put(kanaWritingText, japaneseWordWritingsChecker);
		for (JTextComponent kanjiTextComponent : kanjiTextComponents) {
			propertyManagersOfTextFields.put(kanjiTextComponent, japaneseWordWritingsChecker);
		} if (listPanelViewMode.equals(ListPanelViewMode.VIEW_AND_EDIT)) {
			Stream.concat(kanjiTextComponents.stream(), Stream.of(kanaWritingText)).forEach(
					textComponent -> textComponent.addFocusListener(
							new ListPropertyChangeHandler<>(japaneseWordInformation,
									...
									applicationWindow, japaneseWordWritingsChecker,
									ExceptionsMessages.
		...
		List<JComponent> elementsInRow = new ArrayList<>(); elementsInRow.add(kanaWritingText);
		elementsInRow.addAll(kanjiTextComponents);

		AbstractButton addKanjiWritingButton = createButtonAddKanjiWriting(japaneseWordInformation,
				kanaWritingText, rootPanel, listPanelViewMode);
		AbstractButton removeKanaAndKanjiWritingsButton = createButtonRemoveKanaAndKanjiWritings(
				rootPanel, rootPanel.getNumberOfRows() - 2, kanaWritingText, listPanelViewMode,
				japaneseWordInformation); if (kanaToKanjiWritingsTextComponents.size() == 1) {
		...
		kanaAndKanjiWritings.addRow(SimpleRowBuilder
				.createRow(FillType.NONE, elementsInRow.toArray(new JComponent[] {})));

		rootPanel.insertRowStartingFromColumn(1, listPanelViewMode.equals(ListPanelViewMode.ADD) ?
				rootPanel.getNumberOfRows() - 1 :
				rootPanel.getNumberOfRows() - 2, kanaWritingLabel, kanaAndKanjiWritings.getPanel());
	}

	private AbstractButton createButtonAddKanjiWriting(
			JapaneseWordInformation japaneseWordInformation, JTextComponent kanaWritingText,
			MainPanel panel, ListPanelViewMode viewMode) {
		AbstractButton button = GuiMaker
				.createButtonlikeComponent(ComponentType.BUTTON, ButtonsNames.ADD_KANJI_WRITING,
						null); button.addActionListener(new AbstractAction() {
			@Override public void actionPerformed(ActionEvent e) {
				JTextComponent kanjiTextComponent = CommonGuiElementsMaker
						.createShortInputWithPrompt(Prompts.KANJI_TEXT);
				propertyManagersOfTextFields.put(kanjiTextComponent, japaneseWordWritingsChecker);
				kanaToKanjiWritingsTextComponents.get(kanaWritingText).add(kanjiTextComponent);
				kanjiTextComponent.addFocusListener(
						new ListPropertyChangeHandler<>(japaneseWordInformation,
								applicationWindow.getApplicationController().getJapaneseWords(),
								applicationWindow, japaneseWordWritingsChecker, ExceptionsMessages.
								JAPANESE_WORD_WRITINGS_ALREADY_DEFINED, Prompts.KANJI_TEXT));

				panel.insertElementBeforeOtherElement(button, kanjiTextComponent);
				if (viewMode.equals(ListPanelViewMode.ADD)) {
					Window w = SwingUtilities.windowForComponent(button); if (w != null) {
						w.pack();
					} SwingUtilities.invokeLater(() -> kanjiTextComponent.requestFocusInWindow());
				}
			}
		}); return button;
	}

	private AbstractButton createButtonRemoveKanaAndKanjiWritings(MainPanel panel, int rowNumber,
			JTextComponent kanaTextComponent, ListPanelViewMode viewMode,
		...
		AbstractButton button = GuiMaker.createButtonlikeComponent(ComponentType.BUTTON,
				ButtonsNames.REMOVE_KANA_AND_KANJI_WRITINGS, (new AbstractAction() {
					@Override public void actionPerformed(ActionEvent e) {
						...
						if (viewMode.equals(ListPanelViewMode.ADD)) {
							SwingUtilities.windowForComponent(panel.getPanel()).pack();
	...
	private AbstractButton createButtonAddKanaAndKanjiWritings(
		...
		AbstractButton button = GuiMaker.createButtonlikeComponent(ComponentType.BUTTON,
				ButtonsNames.ADD_KANA_AND_KANJI_WRITINGS, (new AbstractAction() {
					@Override public void actionPerformed(ActionEvent e) {
						addKanaAndKanjiWritingRow(japaneseWordInformation, panel,
								kanaAndKanjiWritingsValues, listPanelViewMode);
						if (listPanelViewMode.equals(ListPanelViewMode.ADD)) {
							Window w = SwingUtilities.
									windowForComponent(panel.getPanel()); if (w != null) {
								w.pack();
							}
						}
					}
