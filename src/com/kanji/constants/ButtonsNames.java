package com.kanji.constants;

public class ButtonsNames {

	public static final String ADD = "Dodaj słówko";

	public static final String APPROVE = "OK";
	public static final String ADD_WORD = "Dodaj";
	public static final String START_LEARNING = "Start";
	public static final String CLOSE_WINDOW = "Zamknij";
	public static final String CANCEL = "Anuluj";
	public static final String FIND_NEXT = "Następny";
	public static final String ADD_ROW = "Dodaj wiersz";
	public static final String NOT_RECOGNIZED = "Nie pamietam";
	public static final String GO_BACK = "Powrót";
	// Buttons
	public static final String START = "Rozpocznij";
	public static final String SEARCH = "Znajdź słówko";
	public static final String OPEN = "Wczytaj listę";
	public static final String SAVE = "Zapisz";
	public static final String SAVE_LIST = "Zapisz listę";
	public static final String FIND_PREVIOUS = "Poprzedni";
	public static final String REMOVE_ROW = "Usuń wiersz";
	public static final String SHOW_KANJI = "Pokaż kanji";
	public static final String RESUME = "Wznów";
	public static final String RECOGNIZED_WORD = "Znam";
	public static final String PREVIOUS_WORD = "Pokaż poprzednie";
	public static final String SHOW_PROBLEMATIC_KANJIS = "Problematyczne kanji";
	public static final String PAUSE = "Pauza";
	public static final String GO_TO_SOURCE = "Przejdź do źródła";

	public static final String CONFIRM = "Tak";
	public static final String REJECT = "Nie";

	public static final String[] BUTTONS_ON_MAIN_PAGE = { START, SEARCH, ADD, OPEN, SAVE,
			SAVE_LIST };

}
