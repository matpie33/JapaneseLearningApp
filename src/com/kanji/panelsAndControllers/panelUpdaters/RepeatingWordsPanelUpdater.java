...
public class RepeatingWordsPanelUpdater {
	...
	public void setButtonsToWordGuessState(boolean previousWordExists) {
		panel.getNotRecognizedWordButton()
			 .setEnabled(false);
		panel.getShowPreviousWordButton()
			 .setEnabled(previousWordExists);
	...
	public void updateRemainingWordsText(int numberOfWordsRemaining,
		...
		panel.getRemainingWordsAmountLabel()
	...
	private String createRemainingWordsText(int numberOfWordsRemaining,
			TypeOfWordForRepeating typeOfWordForRepeating) {
		return String.format(Prompts.WORDS_LEFT_TO_REPEAT,
				JapaneseWritingUtilities.getTextForTypeOfWordForRepeating(
						typeOfWordForRepeating), numberOfWordsRemaining);
	}

	public void updateTime(String timePassed) {
		panel.getTimeElapsedLabel()
			 .setText(Labels.TIME_LABEL + timePassed);
	}

	public void setWordHint(String wordHint) {
		panel.getWordHintTextPane()
			 .setText(wordHint);
	}

	public void toggleGoToPreviousWordButton() {
		AbstractButton showPreviousWordButton = panel.getShowPreviousWordButton();
