...
public class JapaneseWordsFileReader {
	...
	public List <JapaneseWordInformation> readFiles (File [] files) throws IOException,
			IncorrectJapaneseWordsListInputException{
		for (File file: files){
			readFile(file);
		}
		checkForErrors ();
	...
	private List <JapaneseWordInformation> changeStringsToJapaneseWordInformation(){
		...
		for (int i=0; i<wordsInKanaList.size(); i++){
			...
			if (stringInKana.contains("+する") || stringInKanji.contains("+する")
					|| (stringInKana.contains("rzeczownik") && !stringInKana.contains
					("rzeczownik +")) || stringInKana.contains("+の")){
				...
				partOfSpeech = PartOfSpeech.NA_ADJECTIVE;
					...
					.matches(VERB_ENDINGS) || getLast2CharactersOfWord(stringInKana)
					.equals("ない")){
	...
	private String checkForTakingParticleAndModifyString(String wordRepresentation,
			char particle, JapaneseWordInformation japaneseWordInformation){
		...
		for (int i=1; i<splittedWords.length; i++){
			...
			if (partOfSpeech.equals(PartOfSpeech.VERB) && !isLastCharacterKanji &&
				...
				if (!foundVerbConjugationType) {
					japaneseWordInformation.addAditionalInformation(AdditionalInformationTag.VERB_CONJUGATION,
							determineVerbConjugationType(nextWord.charAt(nextWord.length() - 2)).getDisplayedText());
	...
	private void checkForErrors () throws IncorrectJapaneseWordsListInputException {
		...
		if (message.isEmpty() && (wordsInKanaList.size() != meaningsList.size() || wordsInKanjiList.size() != wordsInKanaList
				.size())){
			message += ExceptionsMessages.
