...
public class KanjiListFileReader {
	...
	private final String HEADERS_REGEX =
			KANJIS_HEADER + "|" + REPEATING_DATES_HEADER + "|" + PROBLEMATIC_KANJIS_HEADER;
	...
	public void writeToFile(File f, MyList<KanjiInformation> listOfWords,
		...
		for (RepeatingInformation repeatingDate : repeatingInformations) {
			p.write(repeatingDate.getRepeatingRange() + SEPARATOR + repeatingDate.getRepeatingDate()
					+ SEPARATOR + repeatingDate.getTimeSpentOnRepeating() + SEPARATOR);
			p.newLine();
		}
	...
	private List<KanjiInformation> findWordsAndIDs(File file)
		...
		while (!(line = in.readLine()).matches(HEADERS_REGEX)) {
			...
			if (isIdDuplicated(wordIdInt, kanjiInformations)) {
				...
				kanjiInformations
						.add(new KanjiInformation(kanjiKeyword, Integer.valueOf(wordIdInt)));
			}
		}
	...
	private List<RepeatingInformation> findRepeatingInformations(File file)
		...
		while (!(line = in.readLine()).matches(HEADERS_REGEX)) {
			...
			RepeatingInformation r = new RepeatingInformation(ranges, LocalDateTime.parse(date),
					true, timeSpent);
			information.add(r);
	...
	private void openDesktopAndShowMessage(int wordId) throws DuplicatedWordException, IOException {
		...
		throw new DuplicatedWordException(
				ExceptionsMessages.DUPLICATED_ID_EXCEPTION + " " + NUMBER_TEXT + wordId);
	}
