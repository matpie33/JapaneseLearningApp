...
public class ProblematicKanjisController
		implements ApplicationStateManager, KanjiContextOwner {
	...
	public ProblematicKanjisController(ApplicationWindow applicationWindow,
		...
		this.problematicKanjiPanel = new ProblematicKanjiPanel(kanjiFont,
				kanjiList, applicationWindow, this);
		kanjisToBrowse = new ArrayList<>();
		kanjiCharactersReader = KanjiCharactersReader.getInstance();
		kanjiCharactersReader.loadKanjisIfNeeded();
		kanjiRepeatingList = new MyList<>(applicationWindow,
				applicationWindow.getStartingPanel(), null,
	...
	public void createProblematicKanjisList(
			List<KanjiInformation> reviewedKanjis,
	...
	public void addProblematicKanjis(Set<Integer> problematicKanjisIds) {
		...
		for (Integer kanjiId : problematicKanjisIds) {
			KanjiRow k = new KanjiRow(kanjiId,
					kanjiRepeatingList.getNumberOfWords());
			boolean addedToList = kanjiRepeatingList.addWord(
					new KanjiInformation(kanjiList
							.findRowBasedOnPropertyStartingFromBeginningOfList(
									new KanjiIdChecker(), kanjiId,
	...
	public void goToSpecifiedResource(KanjiRow row) {
		...
		kanjiContext = new KanjiContext(
				kanjiCharactersReader.getKanjiById(row.getId()), row.getId());
		browseKanji(row);
		...
		return new AbstractAction() {
			@Override public void actionPerformed(ActionEvent e) {
				...
				if (hasMoreKanji())
					...
					problematicKanjiPanel.getDialog()
							.showMessageDialog(Prompts.NO_MORE_KANJIS);
				}
			}
		};
	}
	...
	@Override public SavingInformation getApplicationState() {
		ProblematicKanjisState information = new ProblematicKanjisState(
				kanjiRepeatingList.getHighlightedWords(),
				kanjiRepeatingList.getNotHighlightedWords());
	...
	public String getKoohiCookieHeader() {
	...
	public void setKoohiiCookieHeader(String koohiiCookieHeader)
			throws IOException {
		Map<String, List<String>> headers = new LinkedHashMap<>();
		headers.put("Set-Cookie", Arrays.asList(koohiiCookieHeader));
